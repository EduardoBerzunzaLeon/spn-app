...
export const RfcForm = ({ formValues, setFormValues }: RfcFormProps) => {
  const { isLoading } = useQuery(kardexQueries.rfcSearch(formValues));
  ...
  return (
    <form onSubmit={handleSubmit} method="POST">
      <form.AppField name="rfc">
        {(field) => <field.TextField label="RFC" placeholder="Escribe tu RFC" required />}
      </form.AppField>
      <form.AppForm>
        <form.SubmitButton fullWidth mt="xl" label="Buscar" isSubmitting={isLoading} />
      </form.AppForm>
    </form>
  );
};
