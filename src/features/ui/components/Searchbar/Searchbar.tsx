...
export function Searchbar() {
  ...
  const items = useMemo(() => {
    ...
    return Object.keys(itemsObject).map((item) => (
      <Spotlight.ActionsGroup key={item} label={item.trim().toUpperCase()}>
        {itemsObject[item].map((action) => (
          <AppSpotlightLink
            key={action.id}
            label={action.label}
  ...
  return (
    <>
      ...
      <Button variant="default" onClick={spotlight.open} hiddenFrom="sm">
